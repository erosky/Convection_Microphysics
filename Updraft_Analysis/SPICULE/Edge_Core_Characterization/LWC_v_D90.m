...
%     edge_vwind_min = [edge_vwind_min; min(vwind_edge)];
    ...
    fig1 = figure(1);
    for R = 1:height(timestamps)
    scatter(cell2mat(lwcratio_edge(R)), cell2mat(Dratio_edge(R)), 50, "o");
    hold on
    scatter(cell2mat(lwcratio_core(R)), cell2mat(Dratio_core(R)), 50, "filled");
    end
    hold off
    
    ylabel('D90 ratio (normalized by avg core value)');
    xlabel('LWC ratio from CDP (normalized by avg core value)');
    ylim([0.6 1.4])
    grid on
    ...
    fig2 = figure(2);
    for R = 1:height(timestamps)
    scatter(cell2mat(lwc_edge_list(R)), cell2mat(Dratio_edge(R)), 50, "o");
    hold on
    scatter(cell2mat(lwc_core_list(R)), cell2mat(Dratio_core(R)), 50, "filled");
    end
    hold off
    
    ylabel('D90 ratio (normalized by avg core value)');
    xlabel('LWC absolute from CDP');
    ylim([0.6 1.4])
    grid on
    legend({'Edge','Core'}, 'location', 'best')
    

    saveas(fig2, sprintf('%s/%s_LWCvD90_ABSOLUTE.png', output_folder, region));
