...
for i in range(len(regions)):
	...
	ax.text(lonmins[i], latmaxs[i], cloudbases[i], 'cloud base', 'y', verticalalignment="bottom", horizontalalignment="right", color='royalblue', fontsize=12)
	
	ax.plot_surface(X, Y, Ztop, edgecolor='royalblue', lw=0.1, rstride=9, cstride=9, alpha=0.15, linestyle=':',)
	ax.text(lonmins[i], latmaxs[i], cloudtops[i], 'cloud top', 'y', verticalalignment="bottom", horizontalalignment="right", color='royalblue', fontsize=12)
	...
	ax.scatter(region_lons[i], region_lats[i], region_alts[i], s=0.5, color=r_colors, alpha=0.2)
	...
	for p in range(len(track_lats[i])):
		...
		ax.scatter(xline, yline, zline, s=10.0, color=(0.4660, 0.6740, 0.1880), alpha=1.0)
	...
	cb.ax.invert_yaxis()
	ax.grid(False)
	ax.set_title(flightnum[0]);
	ax.set_xlabel('Longitude', fontsize=12, rotation=0, labelpad=10)
	ax.tick_params(axis='x', which='major', labelsize=8, rotation=-15, pad=1)
	ax.tick_params(axis='y', which='major', labelsize=8, rotation=15, pad=1)
	ax.set_ylabel('Latitude', fontsize=12, rotation=10, labelpad=10)
	ax.set_zlabel('Altitude (m)', fontsize=12, rotation=90)
