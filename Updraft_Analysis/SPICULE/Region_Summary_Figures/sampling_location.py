...
x2,y2 = map(-105.00,46.00)
x3,y3 = map(-89.00,46.00)
x4,y4 = map(-89.00,32.70)


poly = Polygon([(x1,y1),(x2,y2),(x3,y3),(x4,y4)],facecolor='None',edgecolor='green',linewidth=1.25)
...
for i in range(len(regions)):
	...
	poly = Polygon([(x1,y1),(x2,y2),(x3,y3),(x4,y4)],facecolor='pink',edgecolor='None')
	plt.gca().add_patch(poly)
	#plt.annotate(flightnum[0]+" "+flightnum[1] + "\n" + datestring[i] + ", 2021", xy=(x4,y4), xytext=(x4+40000,y4-40000), fontsize=8, color='w')

	

plt.savefig('map_full_sampling.png', dpi=1000, bbox_inches='tight')
