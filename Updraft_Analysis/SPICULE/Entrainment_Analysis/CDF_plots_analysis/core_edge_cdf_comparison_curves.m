...
for r=1 : height(timestamps)
   ...
   plot_two_cdfs_continuous(fullfile(core_holo(core_pass).folder, core_holo(core_pass).name), fullfile(edge_holo(edge_pass).folder, edge_holo(edge_pass).name), titlestring, output_folder);
