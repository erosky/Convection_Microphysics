function out = spicule_get_incloud_microphysics()
    ...
    for f = 1:length(Regions)
        % Check if there is EdgeCloud data
        if isfolder(fullfile(Regions(f).folder, Regions(f).name,'/EdgeCloud'))
            % Open up the cloudpass file
            region_folder = fullfile(Regions(f).folder, Regions(f).name,'/EdgeCloud');
