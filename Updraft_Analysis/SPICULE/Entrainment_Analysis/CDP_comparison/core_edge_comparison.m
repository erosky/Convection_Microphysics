function out = core_edge_comparison(region)
...
for r=1 : height(timestamps)
   ...
   % Plot Figure
    ...
    % 90th percentile
    yline(core_90th_pass, '--b','DisplayName', 'Core 90th percentile'); % Mark core 90th percentile
    hold on
    % plot core
    ...
    saveas(fig, sprintf('%s/%s.png', output_folder, passname));
...
output
writetable(output, fullfile(output_folder, 'core_edge_comparison_table.csv'));
